...
// ----------------- Math operations ---------------------
type IterationMap = {
  ['-11']: [-11, '-', '-12', '-10', '11']; ['-10']: [-10, '-', '-11', '-9', '10']; ['-9']: [-9, '-', '-10', '-8', '9'];  ['-8']: [-8, '-', '-9', '-7', '8'];
  ['-7']: [-7, '-', '-8', '-6', '7'];      ['-6']: [-6, '-', '-7', '-5', '6'];     ['-5']: [-5, '-', '-6', '-4', '5'];   ['-4']: [-4, '-', '-5', '-3', '4'];
  ['-3']: [-3, '-', '-4', '-2', '3'];      ['-2']: [-2, '-', '-3', '-1', '2'];     ['-1']: [-1, '-', '-2', '0', '1'];    ['0']: [0, '0', '-1', '1', '0'];
  ['1']: [1, '+', '0', '2', '-1'];         ['2']: [2, '+', '1', '3', '-2'];        ['3']: [3, '+', '2', '4', '-3'];      ['4']: [4, '+', '3', '5', '-4'];
  ['5']: [5, '+', '4', '6', '-5'];         ['6']: [6, '+', '5', '7', '-6'];        ['7']: [7, '+', '6', '8', '-7'];      ['8']: [8, '+', '7', '9', '-8'];
  ['9']: [9, '+', '8', '10', '-9'];        ['10']: [10, '+', '9', '11', '-10'];    ['11']: [11, '+', '10', '12', '-11']; ['12']: [12, '+', '11', '13', '-12'];
  ['13']: [13, '+', '12', '14', '-13'];    ['14']: [14, '+', '13', '15', '-14'];   ['15']: [15, '+', '14', '16', '-15']; ['16']: [16, '+', '15', '17', '-16'];
  ['17']: [17, '+', '16', '18', '-17'];    ['18']: [18, '+', '17', '19', '-18'];   ['19']: [19, '+', '18', '20', '-19']; ['20']: [20, '+', '19', '21', '-20']; 
  ['21']: [21, '+', '20', '22', '-21'];    ['22']: [22, '+', '21', '23', '-22'];   ['23']: [23, '+', '22', '24', '-23']; ['24']: [24, '+', '23', '25', '-24'];
  ['25']: [25, '+', '24', '26', '-25'];    ['26']: [26, '+', '25', '27', '-26'];   ['27']: [27, '+', '26', '28', '-27']; ['28']: [28, '+', '27', '29', '-28'];
  ['29']: [29, '+', '28', '30', '-29'];    ['30']: [30, '+', '29', '31', '-30'];   ['31']: [31, '+', '30', '32', '-31']; ['32']: [32, '+', '31', '33', '-32'];
  ['33']: [33, '+', '32', '34', '-33'];    ['34']: [34, '+', '33', '35', '-34'];   ['35']: [35, '+', '34', '36', '-35']; ['36']: [36, '+', '35', '37', '-36'];
  ['37']: [37, '+', '36', '38', '-37'];    ['38']: [38, '+', '37', '39', '-38'];   ['39']: [39, '+', '38', '40', '-39']; ['40']: [40, '+', '39', '41', '-40'];
}
// @ts-expect-error
...
type EditLastItemAttr<Arr extends any[], Key extends string, Value extends any, T0 = Omit<Last<Arr>, Key> & { [key in Key]: Value }, T1 = { [K in keyof T0]: T0[K]}> = ReplaceLastItem<Arr, T1>
type EditLastItemOfLastItemAttr<Arr extends any[][], Key extends string, Value extends any> = ReplaceLastItem<Arr, EditLastItemAttr<Cast<Last<Arr>, any[]>, Key, Value>>
type PushToLastItem<Arr extends any[][], Item> = ReplaceLastItem<Arr, Push<Cast<Last<Arr>, any[]>, Item>>
// @ts-expect-error
type SplitText<T extends string> = T extends '' ? [] : T extends `${infer First}${infer Rest}` ? [First, ...zeString<Rest>] : T
// --------------- regex parser ------------------
// Stack based parser inspired by this awesome source
// > https://www.youtube.com/watch?v=u01jb8YN2Lw
export type zeString<T> = T extends ''
  ? []
...
export type ParseRegExTokens<T /*extends string[]*/, Stack extends any[] = [[]]> = T extends []
  ...
  ? ParseRegExTokens<
      ...
      Tail<Tail<T>>,
      // @ts-expect-error
      PushToLastItem<Stack, {type: 'element'; value: T[1]; quantifier: 'exactlyOne'}>
      >
  : ParseRegExTokens<Tail<T>, PushToLastItem<Stack, { type: 'element'; value: Head<T>; quantifier: 'exactlyOne' }>>
// @ts-expect-error
type ParsedRegEx<T> = ParseRegExTokens<zeString<T>>[0]
// ------------- regex interpreter ----------------
...
> = Consumed extends 0
  ? [Consumed]
    // @ts-expect-error 
  : [Add<Consumed, ApplyMultipleZeroOrMore<State, Text, Add<Index, Consumed>>[0]>]
...
> =
  State['type'] extends 'wildcard'
    ...
    ? TestRegExp<State['states'], RemoveFirstsNItems<Text, Index>> extends [infer IsMatched, infer Consumed]
...
type TestRegExp<RegExpAST, Text, Index = 0> = RegExpAST extends []
  ...
  ? // TIP: infer is another option how to save value while processing data type
    // @ts-expect-error
    StateMatchesStringAtIndex<Head<RegExpAST>, Text, Index> extends [infer IsMatched, infer Consumed]
    ? IsMatched extends true
      ? TestRegExp<Tail<RegExpAST>, Text, Add<Index, Consumed>>
      : [false, Index, {index: Index; ast: Head<RegExpAST>; text: Text; consumed: Consumed; }]
    : never
  : // @ts-expect-error
  ...
  // @ts-expect-error
  ? ApplyMultipleZeroOrMore<Head<RegExpAST>, Text, Index> extends [infer Consumed]
    ? TestRegExp<Tail<RegExpAST>, Text, Add<Index, Consumed>>
    : TestRegExp<Tail<RegExpAST>, Text, Index>
  : '!!! error!!!'
